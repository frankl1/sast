...
datasets = np.loadtxt(dataset_names_file, dtype=str)[::-1] # every dataset names

to_skip = np.loadtxt('skip-sast-ensemble-full.txt', dtype=str) # model-data to skip 

output_file = 'results-sast-ensemble-full-tail.csv' # the name of the result file

nb_components = 3 # number of component of the ensemble
...
with open(output_file, mode, buffering=1) as f:
	...
	for i, dataset in enumerate(datasets):
		...
		try:
			...
			combination_list = [np.arange(min_shp_length, max_shp_length+1) for i in range(nb_components)]
