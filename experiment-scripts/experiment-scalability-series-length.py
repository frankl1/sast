...
columns = ('series_length','SAST', 'SAST-fixed', 'SASTE', 'SASTE-fixed', 'STC') # columns of the output file

min_shp_length = 3 # min candidate length

output_file = 'results-scalability-series-length.csv' # the name of the result file
...
with open(output_file, mode, buffering=1) as f:
	...
	for i in range(4, max_exponent+1):
		...
		#  Write results
		result = [series_length, sast_time, sast_fixed_time, saste_time, saste_fixed_time, stc_time]
